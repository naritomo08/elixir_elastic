defmodule ElixirElastic.HTML do
  ...
  def render_index(filters, logs, searched, error \\ nil) do
    ...
    <html lang="ja">
      <head>
        ...
        <title>Elixir Elasticsearch Log Search</title>
      ...
      <body>
        <main class="shell">
          <section class="header">
            <div>
              <p class="eyebrow">Elasticsearch Log Search</p>
              <h1>ログのキーワード検索</h1>
              <p class="note">検索結果は条件に一致したログのうち最新50件のみ表示します。</p>
            </div>
          </section>
  ...
  defp search_form(filters) do
    ...
    <form id="search-form" class="search" method="post" action="/">
      #{input("TIME From (JST)", "datetime-local", "time_from", filters["time_from"])}
      #{input("TIME To (JST)", "datetime-local", "time_to", filters["time_to"])}
      <label>
        <span>LOG</span>
        <select name="log_type">
          <option value="">すべて</option>
          #{Enum.map_join(ElasticSearch.log_types(), "", &log_type_option(&1, filters["log_type"]))}
        </select>
      </label>
